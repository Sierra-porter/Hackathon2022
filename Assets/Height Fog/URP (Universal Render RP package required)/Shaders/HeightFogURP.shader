    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings
            {
                float2 uv                       : TEXCOORD0;
                float2 uvLM                     : TEXCOORD1;
                float4 positionWSAndFogFactor   : TEXCOORD2; // xyx: positionWS, w: vertex fog factor
            ...
            float3 DoHeightFogStuff(float3 color, Varyings input) {
                half3 localPos = waveCalc(input.positionOS);
                half3 wPos = waveCalc(input.positionWSAndFogFactor);
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                color *= _Color.rgb * tex2D(_MainTex, input.uv).rgb;
                color = DoHeightFogStuff(color, input);
